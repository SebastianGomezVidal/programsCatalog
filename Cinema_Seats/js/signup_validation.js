...
function banner_button_signup(clicked_id){
...
function pass_data(clicked_from){
	...
	else if (clicked_from == "log_login")
...
function validate_signup() {
	...
	if(checkCookie() && sessionStorage.getItem("banner_display") == "disable"){
			...
			// The project works with a simple password
			/*
			 if(!(password1.match(/[a-z]/g)) || !(password1.match(/[A-Z]/g)) || !(password1.match(/[0-9]/g)) || (password1.length <= 8))
			{
			...
			// The project works with a simple password
			/*
			if(!(password2.match(/[a-z]/g)) || !(password2.match(/[A-Z]/g)) || !(password2.match(/[0-9]/g)) || (password2.length <= 8))
			{
				alert( "The length of the confirmation password should be at least of "+
				"8 alphanumeric characters, contain one lowercase letter, one uppercase letter and one numeric digit");
			...
			{
				alert( "Given passwords are different, please provide matching passwords" );
				document.signup.password1.focus() ;
				document.signup.password2.focus() ;
				event.preventDefault();
				return false;
